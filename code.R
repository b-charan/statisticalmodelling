data = read.csv("C:/Users/asd/Desktop/R/Macromodel/data.csv")
View(data)
attach(data)
library('ggplot2')
library('forecast')
...
model = lm(CIT_REVENUE_Growth_Rate~EXCHANGE_RATE+BOP_INFLOWS_Growth_Rate+BOP_OUTFLOWS_Growth_Rate+LENDING_RATE+CIT_RATE+GDP_PERCAPITA_Growth_Rate+INFLATION_RATE+URBAN_POPULATION_GROWTH_RATE)
...
forecastsEx = forecast(data$EXCHANGE_RATE, h=3)
forecastsEx 
...
forecastsBI = forecast(data$BOP_INFLOWS_Growth_Rate, h=3)
forecastsBI
...
forecastsBO = forecast(data$BOP_OUTFLOWS_Growth_Rate, h=3)
forecastsBO
...
forecastsLr = forecast(data$LENDING_RATE, h=3)
forecastsLr
...
forecastsCr = forecast(data$CIT_RATE, h=3)
forecastsCr
...
forecastsGp = forecast(data$GDP_PERCAPITA_Growth_Rate, h=3)
forecastsGp
...
forecastsIr = forecast(data$INFLATION_RATE, h=3)
forecastsIr
...
forecastsUp = forecast(data$URBAN_POPULATION_GROWTH_RATE, h=3)
forecastsUp
...
data1 = read.csv("C:/Users/asd/Desktop/R/Macromodel/data1.csv")
View(data1)
attach(data1)
